...
| CTDI_vol (mGy), per phase | 🟢 Automatic | Prefer `rdsr.acquisitions[*].mean_ctdivol_mgy` (the true per-irradiation-event value) over `series[*].ctdi_vol_mgy` (the flat per-image `CTDIvol` tag, confirmed **cumulative-to-date** on this scanner, not per-acquisition — see note above). Note the sidecar's `series[*]` is keyed by DICOM series (reconstruction), not by irradiation event — one RDSR acquisition can produce several `series[]` entries at different kernel/thickness — and now *includes* topograms/localizers (flagged `is_topogram`), since a topogram is itself a real dose event. |
